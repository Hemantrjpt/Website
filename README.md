...
```bash
cd client && npm run build   # outputs client/dist — static files
```

The server needs to run as a persistent Node process (`npm start`). Make sure `server/src/data/` (including `uploads/` and `backups/`) is on persistent storage across deploys — that's the entire database now.
